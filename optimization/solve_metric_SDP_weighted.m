function [theta,w_lower,w_upper,aggregate_slack,time] = solve_metric_SDP_alternating(theta_prev,n_vars,D,n_p,Nc,n,m,n_W,delta_wl,eps_wl,w_const,lambda,eps_l,mu_w,mu_s,...
...
%% Define variables, parameters, and constraints
...
for k = 1:Nc
    ...
    eigs_F = [eigs_F; max(eig(F))];
    sum_F = sum_F + gamma(k)*F;
...
opt = optimizer(Constraints, (mu_s/Nc)*(w_upper-w_lower) + (mu_s/Nc)*mu_w*((theta-theta_prev)'*(theta-theta_prev)) + slack, options, ...
                gamma, {theta, w_lower, w_upper, slack});
            
gamma_vals = ones(Nc,1) / Nc;
while true
    [~,~,~,~,~,sol] = opt(gamma_vals);
    if  (~sol.problem)
        theta = double(theta);   
    ...
    eigs_F_vals = double(eigs_F);
    if max(eigs_F_vals) <= max_slack
        break;
    else
        % Update LMI weights based on eigenvalues
        gamma_vals = exp(eigs_F_vals) ./ sum(exp(eigs_F_vals));
    end
end
% fprintf('Done!');
